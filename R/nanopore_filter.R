...
#' @export
nanopore_filter <- function(reads, min_quality = 1, min_length = 200, remove = FALSE) {
  
  if (isFALSE(remove)){
    #Return the row indices to remove. 
    return(append(
                  which(reads$sequence_length_template < min_length), 
                  which(reads$mean_qscore_template < min_quality)))
  } else {
    return(reads[-(append(
      which(reads$sequence_length_template < min_length), 
      which(reads$mean_qscore_template < min_quality))),])
